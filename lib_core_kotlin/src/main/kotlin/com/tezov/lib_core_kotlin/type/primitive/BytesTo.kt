...
object BytesTo {
    ...
    fun UByteArray.toDouble(offset: Int = 0) = this.let {
        assert(LongTo.BYTES == DoubleTo.BYTES, { "long and double doesn't have the same size" })
        var result: Long = 0
    ...
    fun UByteArray.toFloat(offset: Int = 0) = this.let {
        assert(IntTo.BYTES == FloatTo.BYTES, { "int and float doesn't have the same size" })
        var result: Int = 0
